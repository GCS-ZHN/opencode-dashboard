...
- **Per-host backend** = FastAPI aggregator (`server/`). Reads storage via `opencode db`, exposes `API.md` endpoints + SSE `/stream`. **Cached aggregation**: results are computed on first request, then refreshed by a background loop every `poll_seconds` (default 5) into an in-memory `Cache` (in `app.py`, keyed by `(kind, id, since, until)` so time-range variants don't collide); requests serve cached values, a failed refresh keeps the stale value, and `/stream` is driven by the refresh loop via a `StreamHub` (subscribers never query the DB).
...
- `client/` — Node ≥22, `bun`, TypeScript, Vite (vanilla TS, no UI/chart libs). `cli.ts` is pure Node, bundled to `dist-cli/cli.mjs` (bun build, node shebang) — **the CLI runs on node, not bun**. Repo `dashboard.yaml` is dev-only (vite dev proxy + `/api/config` via `vite.config.ts`). Build order: `tsc && tsc -p tsconfig.node.json && vite build && bun build cli.ts --target=node`. Client tests: `bun test mcp.test.mjs export.test.ts range.test.ts auth.test.mjs` (Node ≥22 `node:test`, no framework; test files live at `client/*.test.*`, outside `src/` so the app tsconfig doesn't typecheck them).
- **Front-end PAGE behavior is verified with Playwright** (`client/playwright.config.ts`, specs in `client/e2e/`, run `bun run test:e2e` in `client/`). Any UI change that affects what the user sees — loading states, the time-range control, headers/badges, auth — must be confirmed against a real browser, not just assumed from `bun run build`. The Playwright webServer boots the mock backend (`client/mock-backend.ts`, large slow dataset) + the built front-end server, so tests are self-contained; `bun run test:e2e` builds the CLI first. Auth behavior is covered at the HTTP level in `e2e/auth.spec.ts` (no-auth instance open, configured instance 401 → Basic-accept).
...
- Verify commands before claiming success: server = `uv run pytest -q` + `uvx ruff check .`; client = `bun run build` (tsc + node tsc + vite + cli bundle) + `bun test` (all `client/*.test.*`) +, when the UI changed, `bun run test:e2e`.
